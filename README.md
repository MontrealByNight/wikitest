# Introduction

Welcome to the official website for the Montreal by Night Discord Server, the preeminent server for all your Fifth Edition Vampire the Masquerade, and Play-by-Post desires.&#x20;

Contained herein is the penultimate guide to the server and its’ rules.&#x20;

Here you will find player expectations, character creation rules, guide to the Inconnu bot, as well as a directory for the cities locations, and the Coteries that lay claim to them.



If you're ever unsure about something, then this is the place for you. Still unsure? Feel free to ask a staff member on the server!



# Welcome to Montreal by Night!\
This is a VTM rpg server. We use the rules and the system of VTM v5, with some slight adjustments and houserules to better fit the format and the chronicle.

The game will touch dark subjects, violence, content warnings and so on. There could be graphic description of any of those subjects.

And because of that, the server is strictly a 18+ server. If you are not 18 \[or whatever is the legal age in your own country] please leave the server immediately.

By clicking on the emoji you accept the aforementioned rules and you declare that you are 18+ and you will gain access to the server.

Also, obviously, we are in no way affiliated with White Wolf, Paradox or any copyrighted entity and we can operate under the Dark Pack license.

<iframe src="https://discord.com/widget?id=1131321454701580318&theme=dark" width="350" height="500" allowtransparency="true" frameborder="0" sandbox="allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts"></iframe>

<figure><img src="https://images-ext-1.discordapp.net/external/vL5dRUSiK7QDzs97IRoBJ9FJkNQYFHMbFZhhjzYeaO0/https/cdn-longterm.mee6.xyz/plugins/embeds/images/1131321454701580318/bab49e54d8467f72f1dea73b032122bf8b255bb3db380391ec50454cf68f7525.png?format=webp&#x26;quality=lossless&#x26;width=1586&#x26;height=686" alt=""><figcaption></figcaption></figure>
